...
import {
  checkAvailability,
  resetBookingOperation,
} from "../../../redux/Slices/bookingSlice";
import {
  calculateBookingPrice,
  resetCalculation,
} from "../../../redux/Slices/priceCalculationSlice";
import {
  getTripExtras,
  assignExtraToBooking,
} from "../../../redux/Slices/extrasSlice";
...
const PickupStep = ({ onNext, tripData, availabilityData, childAges }) => {
  ...
  const {
    ...
    error: extrasError,
    tripExtras,
  } = useSelector((state) => state.extras);
  ...
  } = useSelector((state) => state.priceCalculation);
  ...
  const handleExtraCountChange = async (extraId, change) => {
    ...
    if (availabilityData?.idOut) {
      ...
      try {
        ...
        const extra = tripExtras.find(
          (e) => e.facility_id === parseInt(extraId)
        );
        ...
        if (extra) {
          ...
          for (const [id, count] of Object.entries({
            ...
            if (count > 0) {
              const ext = tripExtras.find(
                (e) => e.facility_id === parseInt(id)
              );
              if (ext) {
                extrasList.push({
    ...
    <>
      <div className="pickup-step">
                ...
                <div key={extra.facility_id} className="extra-item">
                  ...
                  <div className="extra-controls">
                    <button
                      ...
                      onClick={() =>
                        handleExtraCountChange(extra.facility_id, -1)
                      }
                      disabled={
                        extraCounts[extra.facility_id] === 0 ||
                        isProcessingExtras
                      }
                    >
                      <FaMinus />
                    ...
                    <button
                      ...
                      disabled={
                        isProcessingExtras ||
                        (extra.pricing_type === 2 &&
                          extraCounts[extra.facility_id] >= 1)
                      }
                    >
                      <FaPlus />
                    </button>
        ...
        <div className="next-button-section">
          ...
          >
            {tripData?.trip_type == 3
              ? t("bookings.pickup.nextBtnAgree")

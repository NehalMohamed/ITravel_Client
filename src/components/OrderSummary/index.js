...
const OrderSummary = ({ availabilityData }) => {
  ...
  const { summaryData, loading, error, shouldRefresh } = useSelector(
        ...
        {summaryData.extras.map((extra, index) => (
          <div key={extra.id || index} className="detail-item">
            ...
            <p className="detail-text">
              {extra.extra_count > 1 ? `${extra.extra_count} × ` : ""}{" "}
              {extra.extra_name}
            </p>
          </div>
        ))}
  ...
  const renderObligatoryExtras = () => {
    if (
      !summaryData?.extras_obligatory ||
      summaryData.extras_obligatory.length === 0
    ) {
      return null;
    }
    ...
    return (
      <>
        {summaryData.extras_obligatory.map((extra, index) => (
          <div key={extra.id || index} className="detail-item">
            <FaPlus className="detail-icon" />
            <p className="detail-text">{extra.extra_name}</p>
          </div>
        ))}
      </>
    );
  ...
  const isTwoWayTransfer =
    summaryData?.is_two_way === true && summaryData?.trip_type === 2;
      ...
      <div className="order-summary-card">
        ...
        <div className="order-summary-content">
          <div className="summary-image">
            <img src={summaryData.default_img} alt={summaryData.trip_name} />
          </div>
          ...
          <div className="summary-content">
            ...
            <div className="summary-details">
              ...
              <div className="detail-item">
                ...
                <p className="detail-text">
                  <strong>{t("bookings.tripDate")}:</strong>{" "}
                  {summaryData.trip_datestr}
                </p>
              </div>
              ...
              {summaryData.trip_return_datestr && (
                <div className="detail-item">
                  <FaCalendarAlt className="detail-icon" />
                  <p className="detail-text">
                    <strong>{t("bookings.returnDate")}:</strong>{" "}
                    {summaryData.trip_return_datestr}
                  </p>
                </div>
              )}
              ...
              {isTwoWayTransfer && (
                <div className="detail-item">
                  ...
                  <p className="detail-text">{t("booking.twoWayTransfer")}</p>
                </div>
              )}
              ...
              <div className="detail-item">
                ...
                <p className="detail-text">{summaryData.booking_status}</p>
              </div>
